# MS-Manmohit-Sandhu/py-kube Docker Image
...
```sh
docker pull manmohitsandhu/py-kube
```
...
```sh
kubectl run --rm -it --image manmohitsandhu/py-kube troubleshooter
```

For local use:

```sh
docker run --rm -it manmohitsandhu/py-kube
```
